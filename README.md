...
需要说明的是，EWC 的核心思想并不绑定 Windsurf，也不绑定 Codex。

为了更准确地讨论这套方法，作者将 EWC 背后的更高层抽象称为 HAOP：Harness-Agent Orchestration Protocol，即“前台工具-后台 Agent 编排协议”。这里的 HAOP 是作者为便于描述这套协作边界而给出的命名，不表示它已经是某个行业标准。

当前仓库以 Windsurf / Cascade + Codex MCP / Codex CLI fallback 作为已经验证过的参考实现，是因为这是作者当前主要使用和验证的工具组合。

从抽象层看，EWC 更像是一种可迁移的 AI coding 多 Agent 编排模式：

- **前台 AI coding harness**：负责用户交互、任务拆分、权限控制、结果审查和最终交付。
- **后台 CLI-capable Agent**：负责长任务、大范围扫描、批量执行、网络兜底和日志输出。
- **明确授权范围**：所有写入都必须限定文件或目录。
- **可复核日志链条**：后台执行应留下 prompt、log 和最终结果，方便前台工具和用户复核。
